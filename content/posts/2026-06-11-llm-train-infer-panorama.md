---
title: "LLM 训练与推理全景账本：一份带数字的工程决策手册"
date: '2026-06-11'
tags:
- LLM
- GPU
...
>
> 全文所有数字要么是**公式推导**（标记为 [理论]），要么是**论文/产品手册的标称值**（标记为 [标称]），要么是**公开 benchmark / 实测均值**（标记为 [实测]）。看到一个数字而没有这三个标签之一，请直接当成"我编的"——不要信。

下面所有节都以 **Llama-3-70B（dense, GQA, 80 层, hidden=8192, KV heads=8, head_dim=128, FFN=28672）** 作为参考模型，必要时会拓展到 7B / 405B / DeepSeek-V3 / GPT-4 量级做对比。
...
| 阶段 | 计算形状 | 算术强度 (FLOPs / Byte) | bound | 典型 MFU [实测] |
|---|---|---|---|---|
...
| DeepSeek-V3（MLA） | 压缩到 ~70 KB / token [标称] | 70 KB | 2.2 GB | 8.7 GB |
...
| 模型/精度 | 权重大小 | H100 (3.35 TB/s) | H200 (4.8 TB/s) | B200 (8 TB/s) [标称] |
|---|---|---|---|---|
...
>
> [实测] vLLM / SGLang 在 H100 上 70B BF16 TP=8、batch=1 的端到端 decode 时延约 **8~12 ms / token**（含 attention、调度、采样开销），和 5.2 ms 的理论下限差 ~2× 是合理工程损耗。
...
| Prefill 1 token | ~0.5 ms [理论] / ~1 ms [实测] | compute-bound |
| Decode 1 token | ~5 ms [理论] / ~10 ms [实测] | memory-bound |
| 1k 输入 + 1k 输出 端到端 | ~10s [实测] | 长尾主要在 decode |
| 每 token 输出 throughput（batch=64） | ~30 tokens/s/user [实测] | concurrency 越高越摊薄 |
...
**Chinchilla 比例 [实测]**：最优训练 token 数 $D \approx 20\,\Theta$，所以训练总 FLOPs：
...
| Llama-3-70B | 15 T | $6.3 \times 10^{24}$ | ~180 H100·days = **6k H100 训 1 个月**（实测 Llama 3 用了 ~6.4k H100 跑 ~50 天 [实测]）|
...
- NVLink 4.0 单向 900 GB/s [标称]，AllReduce 等效带宽 ~400 GB/s [实测]，单机内绰绰有余
- 跨机走 IB NDR（400 Gb/s = 50 GB/s [标称]），AllReduce 跨机会立刻成为瓶颈 → 必须 hierarchical AllReduce
...
| 训练 1.5T token | ~50 天 [实测] | Llama 3 70B 数量级 |
| 单卡日志/秒 throughput | ~3.5k token/s [实测] | H100 BF16 |

---

## 三、GPU：四个关键数能不能背下来

每张训练/推理卡的命运，由四个数决定：**算力、显存容量、显存带宽、互联带宽**。下表全部 [标称]，来源是各家 datasheet。

### 3.1 NVIDIA 主流训推卡

| GPU | BF16 算力 (TFLOPs) | FP8 算力 (TFLOPs) | HBM 容量 | HBM 带宽 (TB/s) | NVLink 单向 (GB/s) | TDP (W) |
|---|---|---|---|---|---|---|
| **A100 80GB SXM** | 312 | — | 80 GB HBM2e | 2.0 | 600 (NVLink 3) | 400 |
| **H100 SXM** | 989 | 1979 | 80 GB HBM3 | 3.35 | 900 (NVLink 4) | 700 |
| **H200 SXM** | 989 | 1979 | 141 GB HBM3e | 4.8 | 900 | 700 |
| **B200 SXM** | 2250 | 4500 | 192 GB HBM3e | 8.0 | 1800 (NVLink 5) | 1000 |
| **GB200 (1×Grace+2×B200)** | 4500 | 9000 | 384 GB HBM3e | 16.0 | C2C: 900 | 2700 |
| **B100** | 1750 | 3500 | 192 GB HBM3e | 8.0 | 1800 | 700 |
| **L40S** | 91.6 | 366 | 48 GB GDDR6 | 0.864 | 无 NVLink | 350 |

> **FP8 不等于 BF16 × 2**。FP8 有 E5M2 / E4M3 两种格式，训练的有效精度损失比例约 1~3%（Llama-3 全 FP8 [实测]），但 attention softmax 经常被迫保留 BF16，所以**端到端 MFU 提升通常只有 1.5×~1.7×，而不是理论的 2×**。
...
> **认知误区五**："多买点卡训练就快了"。错。**训练吞吐 = min(算力, 通信)**。如果你 IB 网络只有 100G，加再多卡也只是堆通信瓶颈。Llama 3 团队在 405B 训练里**专门重写了 NCCL 调度**[实测，Llama 3 paper §3.3]，因为默认 ring AllReduce 在 16k 卡规模根本撑不住。
...
70B 训练为例：
- batch = 4M token，$t_{\text{step}}$ ≈ 4 s [理论]
- bytes/token：纯 text token id（int32）= 4 B；但**实际训练读的是 raw text**，UTF-8 平均 ~4 B/char ≈ **15 B/token** [实测]（含 padding、metadata）
- $B_{\text{IO}}$ = 4M × 15 / 4 ≈ **15 GB/s 单节点**

**这只是 1 个节点**。1024 卡 = 128 节点，全集群读吞吐 = **128 × 15 = ~2 TB/s**。

| 存储方案 | 可用读带宽 [实测] | 70B 训练能否扛住？ |
|---|---|---|
...
- 10k+ 卡，存储是**独立工程团队**而不是"运维一下"。Meta 在 Llama 3 用了 **240 PB Tectonic 存储 + 7000+ 客户端节点**[实测，Llama 3 paper]。
...
- 这意味着 ckpt 必须 **shard + 并行写**，不能单 rank 收集后串行写。Llama 3 用的是**异步 ckpt + 后台 flush**[实测]。
...
**认知误区八**："数据准备是 CSE 学生的活"。错。Llama 3 paper 里关于数据 pipeline 的内容（去重、质量过滤、curriculum、tokenize、pack）**比 attention kernel 的篇幅多 5 倍**[实测]。**真正决定大模型质量的是数据 infra，不是 GPU kernel**。
...
| KV / token (MLA 压缩 [标称]) | ~70 KB | 320 KB | **MoE 省 4.6×** |
| 端到端 decode（batch=1）[实测] | ~30~40 ms / token | ~10 ms / token | dense 快 3× |
| 端到端 decode（batch=128）[实测] | ~50 ms / token, 60+ tok/s/user | ~25 ms / token | dense 快 2× |
...
- DeepSeek-V3 自己的优化（MTP + MLA + FP8）能把 decode 拉到 60+ tok/s [实测]，已经是 MoE 工程的天花板。
...
- DeepSeek 在 V3 论文里专门设计了 **device-limited routing**（每个 token 最多路由到 4 个节点），就是为了**把 All2All 压到节点内 NVLink 域**[实测]；
...
**Ckpt 写入时间**：DeepSeek-V3 一次 ckpt 10.7 TB，要 60 s 内写完需要 **~180 GB/s 集群写带宽**——比 dense 70B 高一个数量级。**异步 ckpt + sharded write 是 MoE 训练的必选项**[实测]。
...
- Llama 3 技术报告（2024）：训练数据规模与系统工程细节，本文 [实测] 数据主要来源
- Chinchilla（Hoffmann et al., 2022）：$D \approx 20\Theta$ 的来源
- Megatron-LM 论文系列：activation 显存公式
- DeepSeek-V3 技术报告（2024）：MLA / device-limited routing / FP8 训练 [实测] 数据来源
- Mixtral of Experts（Jiang et al., 2024）：MoE 推理基线
- NVIDIA H100 / H200 / B200 datasheet：[标称] 算力与带宽
- 本站姊妹篇：
